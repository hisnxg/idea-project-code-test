...
public class CourierDao{
    ...
    //通过快递员ID查询快递员信息
    public static final  String SQL_FIND_BY_COURIER_ID = "SELECT * FROM COURIER WHERE ID=?";
    //快递员录入
    ...
    //用户登录时间更新
    private static final String SQL_UPDATE_COURIER_LOGIN_TIME = "UPDATE COURIER SET LOGINTIME=?WHERE USERPHONE=?";
    //快递员信息的删除
     ...
     * @return 查询的快递信息列表
     */
    public Courier findByCourierPhone(String userPhone) {
        ...
        try {
            ...
            while(resultSet.next()){
                ...
                int dispatches = resultSet.getInt("dispatches");
                Timestamp registrationTime = resultSet.getTimestamp("registrationTime");
                //Timestamp loginTime = resultSet.getTimestamp("loginTime")==null?null:resultSet.getTimestamp("loginTime");
                Timestamp loginTime = resultSet.getTimestamp("loginTime");
                c = new Courier(id,username,userPhone,identifyCard,password,dispatches,registrationTime,loginTime);
                //.add(c);
            }
            return c;
        } catch (SQLException throwables) {
    ...
    public Courier findByCourierId(int id) {
        ...
        //2.    预编译SQL语句
        try {
            state = conn.prepareStatement(SQL_FIND_BY_COURIER_ID);
            //3.    填充参数(可选)
            state.setInt(1,id);
            //4.    执行SQL语句
            resultSet = state.executeQuery();
     ...
     * @return
     */
    public void updateCourierLoginTime(Date loginTime, String userPhone) {
        //1.    获取连接
        Connection conn = DruidUtil.getConnection();
        PreparedStatement state = null;
        //2.    预编译SQL语句
