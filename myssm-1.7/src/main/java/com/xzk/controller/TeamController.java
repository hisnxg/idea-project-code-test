...
//    }
    ...
    public ResultVO<Team> uploadLogo(@RequestParam("logo")MultipartFile myFile, HttpServletRequest request,@PathVariable("id") Integer teamId){
        //文件上传到指定服务器位置
        String path = request.getServletContext().getRealPath("/img/uploadFile");
        //获取原始文件名称
        String originalFilename = myFile.getOriginalFilename();
        //定义文件的新名称：随机生成的名称+原文件的后缀名
        ...
        try {
            ...
            System.out.println("上传成功！"+logoName);
            //将图片名称更新到数据库中
            Team team = teamService.queryById(teamId);
            team.setTeamId(teamId);
